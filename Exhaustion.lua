...
local function UpdateExhaustOverlayAlphas(elapsed)
 ...
 -- Increased to be more noticable
local RATE_ON_FOOT = 0.025 * 3
local RATE_SWIMMING = 0.04 * 1
local RATE_ON_MOUNT = 0.005 * 2
local RATE_IN_COMBAT = 0.05 * 1
...
function WL.HandleExhaustionDecay(elapsed)
    ...
    local rate
    if IsResting() then
        rate = WL.GetSetting("exhaustionInnDecayRate") * 0.3 + 0.2 or 1.5 - 1.0 -- Slower recovery, feels to instant otherwise
    else
        rate = WL.GetSetting("exhaustionDecayRate") * 0.1 + 0.2 or 0.5 - 0.2 -- Slower recovery, feels to instant otherwise
    end
