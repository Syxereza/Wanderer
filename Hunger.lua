    ...
    local EATING_RECOVERY_RATE = 0.4 * 6 -- Increased, no need to consume so many items
    local RESTED_EATING_RECOVERY = 0.6 * 6 -- Increased, no need to consume so many items

    local CHECKPOINT_WORLD = 50 -- Better breakpoints to exclude trainer
    ...
    local function GetHungerMultiplier()
        ...
        if cachedSettings.temperatureEnabled then
            ...
            if temp < 0 then
                tempFactor = 1.0 + (math_abs(temp) / 100) * 1.0 * 5
            end
        end
